...
> 文件路径：[pluginmanager/drivermanager/state/state.go](file:///d:/claude/nomad/client/pluginmanager/drivermanager/state/state.go)
> 总行数：15 行
> 所属包：`state`
...
该文件属于 **驱动管理器状态子包**（`client/pluginmanager/drivermanager/state`），定义驱动管理器的状态数据结构。
