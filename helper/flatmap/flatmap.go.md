...
> 文件路径：[flatmap/flatmap.go](file:///d:/claude/nomad/helper/flatmap/flatmap.go)
> 总行数：131 行
> 所属包：`flatmap`
...
该文件属于 **扁平化映射子包**（`helper/flatmap`），实现嵌套 map 的扁平化处理，将层级结构转换为点分隔的键值对。
...
| `flatten` | - | `prefix string, v reflect.Value, primitiveOnly bool, enteredStruct bool, outp...` | - | [L32](file:///d:/claude/nomad/helper/flatmap/flatmap.go#L32) |
