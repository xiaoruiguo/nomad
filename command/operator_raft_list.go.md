...
> 文件路径：[operator_raft_list.go](file:///d:/claude/nomad/command/operator_raft_list.go)
> 总行数：107 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator raft list-peers`**，功能简述：
...
**简述**：`Display the current Raft peer configuration`

### Name()

**命令名**：`operator raft list-peers`
