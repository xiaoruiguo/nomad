...
> 文件路径：[operator_snapshot_save.go](file:///d:/claude/nomad/command/operator_snapshot_save.go)
> 总行数：172 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator snapshot save`**，功能简述：
...
**简述**：`Saves snapshot of Nomad server state`

### Name()

**命令名**：`operator snapshot save`
