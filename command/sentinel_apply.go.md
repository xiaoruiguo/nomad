...
> 文件路径：[sentinel_apply.go](file:///d:/claude/nomad/command/sentinel_apply.go)
> 总行数：148 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`sentinel apply`**，功能简述：

> Create a new or update existing Sentinel policies
...
**简述**：`Create a new or update existing Sentinel policies`
