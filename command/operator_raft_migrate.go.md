...
> 文件路径：[operator_raft_migrate.go](file:///d:/claude/nomad/command/operator_raft_migrate.go)
> 总行数：133 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator raft migrate-backend`**，功能简述：

> Migrate raft log store from BoltDB to WAL
...
**简述**：`Migrate raft log store from BoltDB to WAL`

### Name()

**命令名**：`operator raft migrate-backend`
