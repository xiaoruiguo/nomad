...
> 文件路径：[scaling_policy_info.go](file:///d:/claude/nomad/command/scaling_policy_info.go)
> 总行数：206 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`scaling policy info`**，功能简述：
...
**简述**：`Display an individual Nomad scaling policy`
...
| [scaling.go](file:///d:/claude/nomad/command/scaling.go) | 父命令文件 |
