...
> 分析文件：[deployment_unblock.go](file:///d:/claude/nomad/command/deployment_unblock.go)
> Run 函数数量：1

### 1. *DeploymentUnblockCommand.Run

**定义位置**：[L74-L143](file:///d:/claude/nomad/command/deployment_unblock.go#L74-L143)

**函数签名**：

```go
func (*DeploymentUnblockCommand) Run(args []string) (int) {
    // ...
}
```
...
| L121 | `client.Deployments().Unblock` | 调用 Deployments API |
| L121 | `client.Deployments` | 业务调用 |
| L142 | `mon.monitor` | 业务调用 |

**涉及的 Nomad API 端点**：

- `Deployments API.Unblock`
