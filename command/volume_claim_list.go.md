...
> 文件路径：[volume_claim_list.go](file:///d:/claude/nomad/command/volume_claim_list.go)
> 总行数：171 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`volume claim list`**，功能简述：

> List existing host volume claims
...
**命令名**：`volume claim list`

### Synopsis()

**简述**：`List existing host volume claims`
