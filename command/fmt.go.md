...
> 文件路径：[fmt.go](file:///d:/claude/nomad/command/fmt.go)
> 总行数：294 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`fmt`**，功能简述：

> Rewrites Nomad config and job files to canonical format
...
**简述**：`Rewrites Nomad config and job files to canonical format`
