...
> 文件路径：[command/operator_raft_info.go](file:///d:/claude/nomad/command/operator_raft_info.go)
> 总行数：77 行
> 所属包：`command`
...
该文件属于 **CLI 命令包**（`command/`），实现 `nomad operator_raft_info` 命令，通过 Nomad API 客户端与 Server 交互，提供作业管理、节点查询、集群运维等命令行功能。

## 2. 类型定义

### OperatorRaftInfoCommand

**定义位置**：[L14](file:///d:/claude/nomad/command/operator_raft_info.go#L14)

**中文说明**：OperatorRaftInfoCommand 与 Raft 共识相关，用于维护集群状态的一致性。
...
**位置**：[L48](file:///d:/claude/nomad/command/operator_raft_info.go#L48)
...
> 分析文件：[operator_raft_info.go](file:///d:/claude/nomad/command/operator_raft_info.go)
> Run 函数数量：1

### 1. *OperatorRaftInfoCommand.Run

**定义位置**：[L48-L76](file:///d:/claude/nomad/command/operator_raft_info.go#L48-L76)

**函数签名**：

```go
func (*OperatorRaftInfoCommand) Run(args []string) (int) {
    // ...
}
```
...
| 行号 | 调用 | 说明 |
|------|------|------|
| L57 | `raftutil.FindRaftStore` | 业务调用 |
| L59 | `err.Error` | 输出错误信息 |
| L63 | `raftutil.RaftStateInfo` | 业务调用 |
| L68 | `store.Close` | 业务调用 |
...
| 行号 | 注释内容 |
|------|---------|
| L56 | Find the raft log store (raft.db or wal/ directory) |
