...
> 文件路径：[operator_gossip_keyring_list.go](file:///d:/claude/nomad/command/operator_gossip_keyring_list.go)
> 总行数：102 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator gossip keyring list`**，功能简述：
...
**简述**：`List gossip encryption keys`
...
**命令名**：`operator gossip keyring list`
