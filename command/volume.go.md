...
> 分析文件：[volume.go](file:///d:/claude/nomad/command/volume.go)
> Run 函数数量：1

### 1. *VolumeCommand.Run

**定义位置**：[L59-L61](file:///d:/claude/nomad/command/volume.go#L59-L61)

**函数签名**：

```go
func (*VolumeCommand) Run(args []string) (int) {
    // ...
}
```
...
| 行号 | 退出代码 | 退出原因 |
|------|---------|---------|
| L60 | `return cli.RunResultHelp` | 返回值 |
