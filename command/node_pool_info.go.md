...
> 文件路径：[node_pool_info.go](file:///d:/claude/nomad/command/node_pool_info.go)
> 总行数：155 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`node pool info`**，功能简述：

> Fetch information about an existing node pool
...
**简述**：`Fetch information about an existing node pool`
