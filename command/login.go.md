...
> 文件路径：[login.go](file:///d:/claude/nomad/command/login.go)
> 总行数：317 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`login`**，功能简述：

> Login to Nomad using an auth method
...
**简述**：`Login to Nomad using an auth method`
