...
> 分析文件：[acl_binding_rule_list.go](file:///d:/claude/nomad/command/acl_binding_rule_list.go)
> Run 函数数量：1

### 1. *ACLBindingRuleListCommand.Run

**定义位置**：[L68-L113](file:///d:/claude/nomad/command/acl_binding_rule_list.go#L68-L113)

**函数签名**：

```go
func (*ACLBindingRuleListCommand) Run(args []string) (int) {
    // ...
}
```
...
| 行号 | 调用 | 说明 |
|------|------|------|
| L70 | `a.Meta.FlagSet` | 创建 flag 解析器 |
| L70 | `a.Name` | 业务调用 |
| L71 | `a.Ui.Output` | 输出信息到用户 |
| L71 | `a.Help` | 业务调用 |
| L81 | `a.Ui.Error` | 输出错误信息 |
| L82 | `a.Ui.Error` | 输出错误信息 |
| L87 | `a.Meta.Client` | 获取 Nomad API 客户端 |
| L89 | `a.Ui.Error` | 输出错误信息 |
| L94 | `client.ACLBindingRules().List` | 业务调用 |
| L94 | `client.ACLBindingRules` | 业务调用 |
| L96 | `a.Ui.Error` | 输出错误信息 |
| L103 | `a.Ui.Error` | 输出错误信息 |
| L103 | `err.Error` | 输出错误信息 |
| L107 | `a.Ui.Output` | 输出信息到用户 |
| L111 | `a.Ui.Output` | 输出信息到用户 |
| L111 | `formatACLBindingRules` | 业务调用 |

**涉及的 Nomad API 端点**：

- `ACL BindingRules API.List`
...
| 行号 | 注释内容 |
|------|---------|
| L79 | Check that we got no arguments |
| L86 | Get the HTTP client |
| L93 | Fetch info on the policy |
