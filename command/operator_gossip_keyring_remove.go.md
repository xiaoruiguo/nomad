...
> 文件路径：[operator_gossip_keyring_remove.go](file:///d:/claude/nomad/command/operator_gossip_keyring_remove.go)
> 总行数：91 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator gossip keyring remove`**，功能简述：

> Remove a gossip encryption key
...
**简述**：`Remove a gossip encryption key`
...
**命令名**：`operator gossip keyring remove`
