...
> 文件路径：[acl_binding_rule_create.go](file:///d:/claude/nomad/command/acl_binding_rule_create.go)
> 总行数：173 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl binding-rule create`**，功能简述：
...
**简述**：`Create a new ACL binding rule`

### Name()

**命令名**：`acl binding-rule create`
