...
> 文件路径：[acl_binding_rule_update.go](file:///d:/claude/nomad/command/acl_binding_rule_update.go)
> 总行数：212 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl binding-rule update`**，功能简述：
...
**简述**：`Update an existing ACL binding rule`

### Name()

**命令名**：`acl binding-rule update`
