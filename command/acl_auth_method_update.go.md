...
> 文件路径：[acl_auth_method_update.go](file:///d:/claude/nomad/command/acl_auth_method_update.go)
> 总行数：243 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl auth-method update`**，功能简述：
...
**简述**：`Update an existing ACL auth method`

### Name()

**命令名**：`acl auth-method update`
