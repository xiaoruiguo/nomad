...
> 文件路径：[acl_bootstrap.go](file:///d:/claude/nomad/command/acl_bootstrap.go)
> 总行数：235 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl bootstrap`**，功能简述：

> Bootstrap the ACL system for initial token
...
**简述**：`Bootstrap the ACL system for initial token`
