...
> 文件路径：[service_info.go](file:///d:/claude/nomad/command/service_info.go)
> 总行数：347 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`service info`**，功能简述：

> Display an individual Nomad service registration
...
| `getServiceByPrefix` | - | `client *api.Services, opts *api.QueryOptions` | `ns string, id string, possible []*api.ServiceRegistrationListStub, err error` | [L290](file:///d:/claude/nomad/command/service_info.go#L290) |
...
**简述**：`Display an individual Nomad service registration`
