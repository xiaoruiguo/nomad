...
> 文件路径：[quota_init.go](file:///d:/claude/nomad/command/quota_init.go)
> 总行数：172 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`quota init`**，功能简述：

> Create an example quota specification file
...
| 名称 | 值 |
|------|----|
| `defaultHclQuotaSpec` | `*ast.CallExpr` |
| `defaultJsonQuotaSpec` | `*ast.CallExpr` |
...
**简述**：`Create an example quota specification file`
