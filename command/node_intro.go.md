...
> 文件路径：[node_intro.go](file:///d:/claude/nomad/command/node_intro.go)
> 总行数：35 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`node intro`**，功能简述：

> Tooling for managing node introduction tokens
...
**简述**：`Tooling for managing node introduction tokens`
