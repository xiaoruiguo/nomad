...
> 文件路径：[acl_token_update.go](file:///d:/claude/nomad/command/acl_token_update.go)
> 总行数：153 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl token update`**，功能简述：
...
**简述**：`Update an existing ACL token`

### Name()

**命令名**：`acl token update`
