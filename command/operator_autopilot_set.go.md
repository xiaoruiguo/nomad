...
> 文件路径：[operator_autopilot_set.go](file:///d:/claude/nomad/command/operator_autopilot_set.go)
> 总行数：173 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator autopilot set-config`**，功能简述：
...
**命令名**：`operator autopilot set-config`
...
**简述**：`Modify the current Autopilot configuration`
