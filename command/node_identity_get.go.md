...
> 文件路径：[node_identity_get.go](file:///d:/claude/nomad/command/node_identity_get.go)
> 总行数：162 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`node identity get`**，功能简述：

> Detail a node's identity claims
...
**简述**：`Detail a node's identity claims`

### Name()

**命令名**：`node identity get`
