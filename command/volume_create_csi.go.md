...
> 文件路径：[command/volume_create_csi.go](file:///d:/claude/nomad/command/volume_create_csi.go)
> 总行数：43 行
> 所属包：`command`
...
该文件属于 **CLI 命令包**（`command/`），实现 `nomad volume_create_csi` 命令，通过 Nomad API 客户端与 Server 交互，提供作业管理、节点查询、集群运维等命令行功能。
