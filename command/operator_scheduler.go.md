...
> 文件路径：[operator_scheduler.go](file:///d:/claude/nomad/command/operator_scheduler.go)
> 总行数：46 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator scheduler`**，功能简述：

> Provides access to the scheduler configuration
...
**简述**：`Provides access to the scheduler configuration`

### Name()

**命令名**：`operator scheduler`
