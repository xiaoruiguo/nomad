...
> 文件路径：[action.go](file:///d:/claude/nomad/command/action.go)
> 总行数：66 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`action`**，功能简述：

> Run a pre-defined command from a given context
...
**简述**：`Run a pre-defined command from a given context`
