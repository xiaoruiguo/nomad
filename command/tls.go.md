...
> 分析文件：[tls.go](file:///d:/claude/nomad/command/tls.go)
> Run 函数数量：1

### 1. *TLSCommand.Run

**定义位置**：[L57-L59](file:///d:/claude/nomad/command/tls.go#L57-L59)

**函数签名**：

```go
func (*TLSCommand) Run(_ []string) (int) {
    // ...
}
```
...
| 行号 | 退出代码 | 退出原因 |
|------|---------|---------|
| L58 | `return cli.RunResultHelp` | 返回值 |
