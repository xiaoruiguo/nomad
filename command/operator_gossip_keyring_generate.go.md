...
> 文件路径：[operator_gossip_keyring_generate.go](file:///d:/claude/nomad/command/operator_gossip_keyring_generate.go)
> 总行数：53 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator gossip keyring generate`**，功能简述：
...
**简述**：`Generates a new encryption key`
...
**命令名**：`operator gossip keyring generate`
