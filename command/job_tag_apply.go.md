...
> 文件路径：[job_tag_apply.go](file:///d:/claude/nomad/command/job_tag_apply.go)
> 总行数：154 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`job tag apply`**，功能简述：

> Save a job version to prevent it from being garbage-collected and allow it to be diffed and reverted by name.
...
**简述**：`Save a job version to prevent it from being garbage-collected and allow it to be diffed and reverted by name.`
