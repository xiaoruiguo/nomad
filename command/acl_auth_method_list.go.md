...
> 文件路径：[acl_auth_method_list.go](file:///d:/claude/nomad/command/acl_auth_method_list.go)
> 总行数：129 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl auth-method list`**，功能简述：

> List ACL auth methods
...
**简述**：`List ACL auth methods`

### Name()

**命令名**：`acl auth-method list`
