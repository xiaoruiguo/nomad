...
> 文件路径：[config_validate.go](file:///d:/claude/nomad/command/config_validate.go)
> 总行数：93 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`config validate`**，功能简述：
...
**简述**：`Validate config files/directories`
...
| [config.go](file:///d:/claude/nomad/command/config.go) | 父命令文件 |
