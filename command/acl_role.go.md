...
> 分析文件：[acl_role.go](file:///d:/claude/nomad/command/acl_role.go)
> Run 函数数量：1

### 1. *ACLRoleCommand.Run

**定义位置**：[L67-L67](file:///d:/claude/nomad/command/acl_role.go#L67-L67)

**函数签名**：

```go
func (*ACLRoleCommand) Run(_ []string) (int) {
    // ...
}
```
...
| 行号 | 退出代码 | 退出原因 |
|------|---------|---------|
| L67 | `func (a *ACLRoleCommand) Run(_ []string) int { return cli.RunResultHelp }` | 返回值 |
