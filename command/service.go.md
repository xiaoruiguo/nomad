...
> 分析文件：[service.go](file:///d:/claude/nomad/command/service.go)
> Run 函数数量：1

### 1. *ServiceCommand.Run

**定义位置**：[L43-L43](file:///d:/claude/nomad/command/service.go#L43-L43)

**函数签名**：

```go
func (*ServiceCommand) Run(_ []string) (int) {
    // ...
}
```
...
| 行号 | 退出代码 | 退出原因 |
|------|---------|---------|
| L43 | `func (c *ServiceCommand) Run(_ []string) int { return cli.RunResultHelp }` | 返回值 |
