...
> 文件路径：[command/acl_policy.go](file:///d:/claude/nomad/command/acl_policy.go)
> 总行数：75 行
> 所属包：`command`
...
该文件属于 **CLI 命令包**（`command/`），实现 `nomad acl_policy` 命令，通过 Nomad API 客户端与 Server 交互，提供作业管理、节点查询、集群运维等命令行功能。

## 2. 类型定义

### ACLPolicyCommand

**定义位置**：[L15](file:///d:/claude/nomad/command/acl_policy.go#L15)

**中文说明**：ACLPolicyCommand 与访问控制列表（ACL）相关，管理权限和认证。
...
**位置**：[L51](file:///d:/claude/nomad/command/acl_policy.go#L51)
...
> 分析文件：[acl_policy.go](file:///d:/claude/nomad/command/acl_policy.go)
> Run 函数数量：1

### 1. *ACLPolicyCommand.Run

**定义位置**：[L51-L53](file:///d:/claude/nomad/command/acl_policy.go#L51-L53)

**函数签名**：

```go
func (*ACLPolicyCommand) Run(args []string) (int) {
    // ...
}
```
