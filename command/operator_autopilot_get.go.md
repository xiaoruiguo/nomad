...
> 文件路径：[operator_autopilot_get.go](file:///d:/claude/nomad/command/operator_autopilot_get.go)
> 总行数：108 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator autopilot get-config`**，功能简述：
...
**命令名**：`operator autopilot get-config`
...
**简述**：`Display the current Autopilot configuration`
