...
> 文件路径：[operator_raft_remove.go](file:///d:/claude/nomad/command/operator_raft_remove.go)
> 总行数：93 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator raft remove-peer`**，功能简述：
...
**简述**：`Remove a Nomad server from the Raft configuration`

### Name()

**命令名**：`operator raft remove-peer`
