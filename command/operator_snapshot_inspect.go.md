...
> 分析文件：[operator_snapshot_inspect.go](file:///d:/claude/nomad/command/operator_snapshot_inspect.go)
> Run 函数数量：1

### 1. *OperatorSnapshotInspectCommand.Run

**定义位置**：[L96-L169](file:///d:/claude/nomad/command/operator_snapshot_inspect.go#L96-L169)

**函数签名**：

```go
func (*OperatorSnapshotInspectCommand) Run(args []string) (int) {
    // ...
}
```
...
| L150 | `humanize.IBytes` | 业务调用 |
| L160 | `humanize.IBytes` | 业务调用 |
| L164 | `humanize.IBytes` | 业务调用 |
...
| 行号 | 注释内容 |
|------|---------|
| L107 | Check that we either got no filename or exactly one. |
| L130 | format as JSON if requested |
| L146 | print human-readable output |
