...
> 文件路径：[server_force_leave.go](file:///d:/claude/nomad/command/server_force_leave.go)
> 总行数：111 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`server force-leave`**，功能简述：
...
**简述**：`Force a server into the 'left' state`
...
| [server.go](file:///d:/claude/nomad/command/server.go) | 父命令文件 |
