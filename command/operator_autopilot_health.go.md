...
> 文件路径：[operator_autopilot_health.go](file:///d:/claude/nomad/command/operator_autopilot_health.go)
> 总行数：189 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator autopilot health`**，功能简述：

> Display the current Autopilot health
...
**简述**：`Display the current Autopilot health`
