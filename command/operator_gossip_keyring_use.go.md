...
> 分析文件：[operator_gossip_keyring_use.go](file:///d:/claude/nomad/command/operator_gossip_keyring_use.go)
> Run 函数数量：1

### 1. *OperatorGossipKeyringUseCommand.Run

**定义位置**：[L55-L90](file:///d:/claude/nomad/command/operator_gossip_keyring_use.go#L55-L90)

**函数签名**：

```go
func (*OperatorGossipKeyringUseCommand) Run(args []string) (int) {
    // ...
}
```
...
| L84 | `client.Agent().UseKey` | 调用 Agent API |
