...
> 分析文件：[license.go](file:///d:/claude/nomad/command/license.go)
> Run 函数数量：1

### 1. *LicenseCommand.Run

**定义位置**：[L43-L45](file:///d:/claude/nomad/command/license.go#L43-L45)

**函数签名**：

```go
func (*LicenseCommand) Run(args []string) (int) {
    // ...
}
```
...
| 行号 | 退出代码 | 退出原因 |
|------|---------|---------|
| L44 | `return cli.RunResultHelp` | 返回值 |
