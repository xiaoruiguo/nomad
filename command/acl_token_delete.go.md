...
> 文件路径：[acl_token_delete.go](file:///d:/claude/nomad/command/acl_token_delete.go)
> 总行数：82 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl token delete`**，功能简述：
...
**简述**：`Delete an existing ACL token`

### Name()

**命令名**：`acl token delete`
