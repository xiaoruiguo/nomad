...
> 分析文件：[operator.go](file:///d:/claude/nomad/command/operator.go)
> Run 函数数量：1

### 1. *OperatorCommand.Run

**定义位置**：[L35-L37](file:///d:/claude/nomad/command/operator.go#L35-L37)

**函数签名**：

```go
func (*OperatorCommand) Run(args []string) (int) {
    // ...
}
```
...
| 行号 | 退出代码 | 退出原因 |
|------|---------|---------|
| L36 | `return cli.RunResultHelp` | 返回值 |
