...
> 文件路径：[node_pool.go](file:///d:/claude/nomad/command/node_pool.go)
> 总行数：125 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`node pool`**，功能简述：

> Interact with node pools
...
| `nodePoolPredictor` | - | `factory ApiClientFactory, filter **ast.IndexExpr` | `complete.Predictor` | [L74](file:///d:/claude/nomad/command/node_pool.go#L74) |
