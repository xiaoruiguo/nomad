...
> 文件路径：[eval_delete.go](file:///d:/claude/nomad/command/eval_delete.go)
> 总行数：334 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`eval delete`**，功能简述：

> Delete evaluations by ID or using a filter
...
**简述**：`Delete evaluations by ID or using a filter`
