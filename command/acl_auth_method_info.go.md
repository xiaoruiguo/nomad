...
> 文件路径：[acl_auth_method_info.go](file:///d:/claude/nomad/command/acl_auth_method_info.go)
> 总行数：119 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl auth-method info`**，功能简述：

> Fetch information on an existing ACL auth method
...
**简述**：`Fetch information on an existing ACL auth method`

### Name()

**命令名**：`acl auth-method info`
