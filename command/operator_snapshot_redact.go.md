...
> 文件路径：[operator_snapshot_redact.go](file:///d:/claude/nomad/command/operator_snapshot_redact.go)
> 总行数：96 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator snapshot redact`**，功能简述：

> Redacts an existing snapshot of Nomad server state
...
**简述**：`Redacts an existing snapshot of Nomad server state`

### Name()

**命令名**：`operator snapshot redact`
