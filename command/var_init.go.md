...
> 文件路径：[var_init.go](file:///d:/claude/nomad/command/var_init.go)
> 总行数：216 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`var init`**，功能简述：

> Create an example variable specification file
...
| 名称 | 值 |
|------|----|
| `defaultHclVarSpec` | `*ast.BinaryExpr` |
| `defaultJsonVarSpec` | `*ast.BinaryExpr` |
...
**简述**：`Create an example variable specification file`
