...
> 文件路径：[acl_auth_method_delete.go](file:///d:/claude/nomad/command/acl_auth_method_delete.go)
> 总行数：90 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl auth-method delete`**，功能简述：
...
**简述**：`Delete an existing ACL auth method`

### Name()

**命令名**：`acl auth-method delete`
