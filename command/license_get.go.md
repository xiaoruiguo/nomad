...
> 分析文件：[license_get.go](file:///d:/claude/nomad/command/license_get.go)
> Run 函数数量：1

### 1. *LicenseGetCommand.Run

**定义位置**：[L49-L71](file:///d:/claude/nomad/command/license_get.go#L49-L71)

**函数签名**：

```go
func (*LicenseGetCommand) Run(args []string) (int) {
    // ...
}
```
...
| L50 | `c.Name` | 业务调用 |
| L51 | `c.Help` | 业务调用 |
| L58 | `c.Meta.Client` | 获取 Nomad API 客户端 |
| L64 | `client.Operator().LicenseGet` | 调用 Operator API |
| L64 | `client.Operator` | 业务调用 |
...
| L61 | `return 1` | 错误退出 |
| L67 | `return 1` | 错误退出 |
| L70 | `return OutputLicenseReply(c.Ui, resp)` | 返回值 |
