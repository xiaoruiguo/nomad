...
> 文件路径：[license_get.go](file:///d:/claude/nomad/command/license_get.go)
> 总行数：72 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`license get`**，功能简述：

> Retrieve the current Nomad Enterprise License
...
**简述**：`Retrieve the current Nomad Enterprise License`
...
| [license.go](file:///d:/claude/nomad/command/license.go) | 父命令文件 |
