...
> 文件路径：[acl_binding_rule_info.go](file:///d:/claude/nomad/command/acl_binding_rule_info.go)
> 总行数：116 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl binding-rule info`**，功能简述：

> Fetch information on an existing ACL binding rule
...
**简述**：`Fetch information on an existing ACL binding rule`

### Name()

**命令名**：`acl binding-rule info`
