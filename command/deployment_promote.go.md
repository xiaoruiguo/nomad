...
> 分析文件：[deployment_promote.go](file:///d:/claude/nomad/command/deployment_promote.go)
> Run 函数数量：1

### 1. *DeploymentPromoteCommand.Run

**定义位置**：[L87-L161](file:///d:/claude/nomad/command/deployment_promote.go#L87-L161)

**函数签名**：

```go
func (*DeploymentPromoteCommand) Run(args []string) (int) {
    // ...
}
```
...
| L131 | `formatDeployments` | 业务调用 |
| L137 | `client.Deployments().PromoteAll` | 调用 Deployments API |
| L137 | `client.Deployments` | 业务调用 |
| L139 | `client.Deployments().PromoteGroups` | 调用 Deployments API |
| L139 | `client.Deployments` | 业务调用 |
...
| L151 | `return 0` | 成功退出 |
| L156 | `return 0` | 成功退出 |
| L160 | `return mon.monitor(u.EvalID)` | 返回值 |
