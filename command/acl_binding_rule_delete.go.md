...
> 分析文件：[acl_binding_rule_delete.go](file:///d:/claude/nomad/command/acl_binding_rule_delete.go)
> Run 函数数量：1

### 1. *ACLBindingRuleDeleteCommand.Run

**定义位置**：[L53-L88](file:///d:/claude/nomad/command/acl_binding_rule_delete.go#L53-L88)

**函数签名**：

```go
func (*ACLBindingRuleDeleteCommand) Run(args []string) (int) {
    // ...
}
```
...
| L79 | `client.ACLBindingRules().Delete` | 业务调用 |
| L79 | `client.ACLBindingRules` | 业务调用 |
| L81 | `a.Ui.Error` | 输出错误信息 |
| L86 | `a.Ui.Output` | 输出信息到用户 |

**涉及的 Nomad API 端点**：

- `ACL BindingRules API.Delete`
...
| 行号 | 注释内容 |
|------|---------|
| L62 | Check that the last argument is the rule ID to delete. |
| L71 | Get the HTTP client. |
| L78 | Delete the specified ACL binding rule. |
