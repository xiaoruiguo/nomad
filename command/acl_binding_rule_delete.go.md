...
> 文件路径：[acl_binding_rule_delete.go](file:///d:/claude/nomad/command/acl_binding_rule_delete.go)
> 总行数：89 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl binding-rule delete`**，功能简述：

> Delete an existing ACL binding rule
...
**简述**：`Delete an existing ACL binding rule`

### Name()

**命令名**：`acl binding-rule delete`
