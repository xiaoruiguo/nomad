...
> 文件路径：[acl_token_create.go](file:///d:/claude/nomad/command/acl_token_create.go)
> 总行数：274 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl token create`**，功能简述：
...
**简述**：`Create a new ACL token`

### Name()

**命令名**：`acl token create`
