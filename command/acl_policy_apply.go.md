...
> 文件路径：[acl_policy_apply.go](file:///d:/claude/nomad/command/acl_policy_apply.go)
> 总行数：170 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl policy apply`**，功能简述：
...
**简述**：`Create or update an ACL policy`

### Name()

**命令名**：`acl policy apply`
