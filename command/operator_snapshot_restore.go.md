...
> 文件路径：[operator_snapshot_restore.go](file:///d:/claude/nomad/command/operator_snapshot_restore.go)
> 总行数：99 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator snapshot restore`**，功能简述：
...
**简述**：`Restore snapshot of Nomad server state`

### Name()

**命令名**：`operator snapshot restore`
