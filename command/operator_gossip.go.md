...
> 文件路径：[operator_gossip.go](file:///d:/claude/nomad/command/operator_gossip.go)
> 总行数：34 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator gossip`**，功能简述：

> Provides access to the Gossip protocol
...
**简述**：`Provides access to the Gossip protocol`
