...
> 文件路径：[acl_auth_method_create.go](file:///d:/claude/nomad/command/acl_auth_method_create.go)
> 总行数：213 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`acl auth-method create`**，功能简述：
...
**简述**：`Create a new ACL auth method`

### Name()

**命令名**：`acl auth-method create`
