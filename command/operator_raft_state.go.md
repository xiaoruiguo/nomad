...
> 文件路径：[operator_raft_state.go](file:///d:/claude/nomad/command/operator_raft_state.go)
> 总行数：110 行
> 所属包：`command`
...
该文件实现 Nomad CLI 命令 **`operator raft state`**，功能简述：
...
**简述**：`Display raft server state`

### Name()

**命令名**：`operator raft state`
